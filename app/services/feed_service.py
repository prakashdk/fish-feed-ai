...
def export_feed_session(include_all: bool = True):
    ...
    for session in feed_sessions:
        data: Dict[str, float | datetime | None] = {
            ...
            "water_pollution_level": session.features["water_pollution_level"],
            "raining": session.features["raining"],
            "predicted_feed": session.final_feed,
            "actual_feed": session.actual_feed,
            "baseline_feed": session.baseline_feed,
        }

        if include_all:
            data["adjustment_factor"] = session.adjustment_factor
            data["ph"] = session.other_factors["ph"]
            data["turbidity"] = session.other_factors["turbidity"]
            data["tds"] = session.other_factors["tds"]
            data["leftover_feed"] = session.leftover_feed
            data["actual_adjustment_factor"] = session.actual_adjustment_factor
            data["feeded_at"] = session.feeded_at
